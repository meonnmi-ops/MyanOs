...
class MyanosDesktop {
    ...
    // ── Real Terminal (connects to MMR Shell via API) ──
    async renderTerminal(body, winId) {
        ...
        try {
            const res = await fetch('/api/exec', {
                ...
                body: JSON.stringify({cmd: 'echo MMRSHELL_OK'}),
            });
            const data = await res.json();
            if (data.output && data.output.includes('MMRSHELL_OK')) {
                apiWorking = true;
            }
        } catch(e) {
            apiWorking = false;
        ...
        if (apiWorking) {
            // Real API mode
            term.innerHTML = `<div class="term-line" style="color:#9ece6a;">✓ MMR Shell v1.0.0 connected</div>
                <div class="term-line" style="color:#565f89;">Type 'help' for commands | 'myan help' for package manager</div>`;
            this._termApiMode = true;
        } else {
            // Fallback: show help
            term.innerHTML = `<div class="term-line" style="color:#f7768e;">⚠ MMR Shell API not available</div>
                <div class="term-line" style="color:#565f89;">Running: python3 server.py (in myanos-build directory)</div>
                <div class="term-line" style="color:#565f89;">Then refresh the page. Using basic mode for now.</div>
    ...
    _addTermInput(term, winId) {
        ...
        input.addEventListener('keydown', async (e) => {
            if (e.key === 'Enter') {
                ...
                input.disabled = true;
                if (cmd) await this._execTermCmd(term, cmd, winId);
    ...
    async _execTermCmd(term, cmd, winId) {
        ...
        out.style.fontFamily = '"JetBrains Mono", "Fira Code", monospace';
        out.style.fontSize = '13px';
        out.style.lineHeight = '1.4';

        // Check local commands (don't send to API)
        const localCmds = {
        ...
        if (this._termApiMode) {
            ...
            try {
                const res = await fetch('/api/exec', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
                    body: JSON.stringify({cmd: cmd, session: winId}),
                });
                const data = await res.json();

                if (data.output === '__CLEAR__') {
                    term.innerHTML = '';
                } else if (data.output === 'exit') {
                    this.closeWindow(winId);
                    return;
                } else if (data.output) {
                    out.innerHTML = this._renderAnsi(data.output);
                }
            } catch(e) {
                out.textContent = `[ERR] API error: ${e.message}`;
                out.style.color = '#f7768e';
            }
        } else {
            // Basic fallback (limited)
            out.textContent = `[API unavailable] Run: python3 server.py`;
            out.style.color = '#f7768e';
        }
    ...
    _renderAnsi(text) {
        ...
        const colors = {
            '\x1b[0;31m': '#f7768e',   // red
            '\x1b[0;32m': '#9ece6a',   // green
            '\x1b[1;33m': '#e0af68',   // yellow
            '\x1b[0;34m': '#7aa2f7',   // blue
            '\x1b[0;35m': '#bb9af7',   // magenta
            '\x1b[0;36m': '#7dcfff',   // cyan
            '\x1b[1;37m': '#c0caf5',   // white bold
            '\x1b[2m': '#565f89',       // dim
            '\x1b[1m': '',               // bold
            '\x1b[0m': '#a9b1d6',      // reset
        };
        let html = text;
        for (const [ansi, color] of Object.entries(colors)) {
            html = html.split(ansi).join(`</span><span style="color:${color}">`);
        }
        html = `<span style="color:#a9b1d6">${html}</span>`;
        return html;
    }
    ...
    _tabComplete(input) {
        ...
        if (parts.length === 1) {
            const matches = builtins.filter(c => c.startsWith(parts[0]));
